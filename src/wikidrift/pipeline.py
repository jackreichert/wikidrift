...
from . import config, drift, prerank, stance, l5_crosslingual, l5_factcheck, mscore
...
def run(article, llm=False, corroborate=False, provider=None, model=None, base_url=None):
    ...
    if l2_leads:
        if llm:
            ...
            try:
                stance.stance_over_time(article, client=client)
                l2_done = True
    ...
    if llm:
        for name, fn in (("crosslingual", l5_crosslingual.crosslingual), ("factcheck", l5_factcheck.factcheck)):
            try:
                print()
                l5[name] = fn(article, client=client)
            except Exception as e:                          # noqa: BLE001
    ...
    return {"article": article, "l1": label, "leads": leads, "l2_adjudicated": l2_done, "mscore": m, "l5": l5}
